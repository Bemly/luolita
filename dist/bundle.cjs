...
esbuild.build({
  ...
  // Inject browser shims at the very beginning of the bundle
  var shimCode = [
    // process shim
  ...
  // Replace the dynamic require interceptor with stub-aware version
  // The exact throw statement in the minified bundle
  var searchStr = "throw Error('Dynamic require of \"'+e+\"' is not supported')";
  var stubObj = '{' + Object.keys(stubs).map(function(m) {
    return '"' + m + '":' + stubs[m];
  }).join(',') + '}';
  var replaceStr = 'var s=' + stubObj + ';if(s[e])return s[e];throw Error(\'Dynamic require of "\'+e+\'" is not supported\')';

  if (code.indexOf(searchStr) !== -1) {
    code = code.split(searchStr).join(replaceStr);
    console.log('Replaced dynamic require interceptor with stub lookup');
  } else {
    console.warn('Warning: dynamic require pattern not found. Bundle may still fail.');
  }
  ...
  // Also check for remaining Dynamic require throws
  const dynRequires = final.match(/Dynamic require of/g) || [];
  console.log('Remaining dynamic require throws:', dynRequires.length ? dynRequires.length + ' occurrences' : 'none (all stubbed)');
